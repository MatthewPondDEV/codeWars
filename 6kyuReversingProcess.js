...
function decode(r) {
    ...
    let letterArr = r.replace(/[0-9]/g, '').split('')
    let numVal = Number(r.replace(/\D/g,''))
    let newStr = ''
    
    letterArr.forEach((val) => {
      
      for(let i = 0; i < 26; i++) {
        if ((i * numVal) % 26 == alphStr.indexOf(val))  {
          newStr = newStr + alphStr.charAt(i)
        }
      }
      })
  
    if (newStr.length != letterArr.join('').length) {
